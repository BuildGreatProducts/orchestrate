...
export default function BranchSwitcher({
  ...
  const runningAgentCount = useTerminalStore(
    (s) =>
      s.tabs.filter((t) => t.projectFolder === projectFolder && !t.exitCode && t.exitCode !== 0)
        .length
  )
      ...
      {open &&
        createPortal(
          <div
            ref={dropdownRef}
            style={style}
            className="w-56 overflow-hidden rounded-md bg-zinc-800 shadow-xl"
          >
            {/* Search / filter */}
